...
export function QuestionPreviewDrawer({ open, onClose, question }) {
  ...
  useEffect(() => {
    if (open) setLang(translations[0]?.language);
        ...
        <>
          ...
          {options.length > 0 && (
            <div className="space-y-2 mb-6">
              {options.map((opt, i) => {
                const isCorrect =
                  correct.correctOptionId === opt.id || (correct.correctOptionIds || []).includes(opt.id);
                return (
                  <div
                    key={opt.id || i}
          ...
          {question.questionType === 'NUMERICAL' && correct.value !== undefined && (
            <div className="mb-6 rounded-lg border border-emerald-300 bg-emerald-50 px-3 py-2.5 text-sm text-emerald-800">
              Correct value: <strong>{correct.value}</strong>
